...
class TornadoPredictionBatch:
    ...
    touchdown_location: Optional[torch.Tensor] = None  # (B,2) - computed from location_offset
    path_trajectory: Optional[torch.Tensor] = None     # (B,3,2) - computed from predictions
    ...
    def items(self):
        """Return key-value pairs for compatibility with dictionary-like usage."""
        return [
            ('tornado_probability', self.tornado_probability),
            ('ef_scale_probs', self.ef_scale_probs),
            ('most_likely_ef_scale', self.most_likely_ef_scale),
            ('location_offset', self.location_offset),
            ('timing_predictions', self.timing_predictions),
            ('uncertainty_scores', self.uncertainty_scores),
            ('radar_signatures', self.radar_signatures),
            ('atmospheric_indicators', self.atmospheric_indicators),
            ('logits', self.logits),
            ('touchdown_location', self.touchdown_location),
...
class TornadoSuperPredictor(nn.Module):
    ...
    def forward(self, radar_x: torch.Tensor, atmo: Dict[str, torch.Tensor]) -> TornadoPredictionBatch:
        ...
        return TornadoPredictionBatch(
            ...
            logits=logits,
            touchdown_location=loc,  # For now, use location_offset as touchdown_location
            path_trajectory=tim.unsqueeze(-1).expand(-1, -1, 2),  # Mock path trajectory from timing
        )
...
# Global tornado prediction engine instance
tornado_prediction_engine = TornadoSuperPredictor()
