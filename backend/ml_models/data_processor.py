...
class AtmosphericDataProcessor:
    ...
    async def get_atmospheric_conditions(self, station_location: Dict[str, float]) -> Dict[str, torch.Tensor]:
        ...
        # Try opening surface data with different filter options
        try:
            ...
            if "multiple values" in str(e):
                try:
                    ds_sfc = xr.open_dataset(surf_path, engine="cfgrib",
                                             backend_kwargs={"indexpath": "", "filter_by_keys": {"stepType": "instant", "typeOfLevel": "surface"}})
                except:
                    try:
                        ds_sfc = xr.open_dataset(surf_path, engine="cfgrib",
                                                 backend_kwargs={"indexpath": "", "filter_by_keys": {"stepType": "max", "typeOfLevel": "surface"}})
                    except:
                        # Fallback to opening without filter
                        ds_sfc = xr.open_dataset(surf_path, engine="cfgrib")

        # Try opening isobaric data with different filter options
        try:
            ...
            if "multiple values" in str(e):
                try:
                    ds_iso = xr.open_dataset(iso_path, engine="cfgrib",
                                             backend_kwargs={"indexpath": "", "filter_by_keys": {"stepType": "instant", "typeOfLevel": "isobaricInhPa"}})
                except:
                    try:
                        ds_iso = xr.open_dataset(iso_path, engine="cfgrib",
                                                 backend_kwargs={"indexpath": "", "filter_by_keys": {"stepType": "max", "typeOfLevel": "isobaricInhPa"}})
                    except:
                        # Fallback to opening without filter
                        ds_iso = xr.open_dataset(iso_path, engine="cfgrib")

        # If we still can't open, use mock data
        if ds_sfc is None or ds_iso is None:
            logger.warning("Using fallback atmospheric data due to GRIB parsing issues")
            return self._create_mock_atmospheric_data(station_location)
        ...
        if latn is None or lonn is None:
            ...
            if latn is None or lonn is None:
                logger.warning("Using fallback atmospheric data due to missing lat/lon coordinates")
                return self._create_mock_atmospheric_data(station_location)
    ...
    def _choose_recent_run(self) -> Dict[str, str]:
        ...
        hh = f"{yesterday.hour:02d}"
        # Try forecast hour 00 first, then 01
        for fxx in ["00", "01"]:
    ...
    async def _ensure_hrrr_files(self, run: Dict[str,str]) -> Tuple[str,str]:
        ...
        if not (os.path.exists(iso_path) and os.path.getsize(iso_path) > 10_000):
            ...
            if not ok and run["fxx"] == "00":
                ok = await self._download_hrrr_file(ymd, hh, "01", "prs", iso_path)
                if ok: run["fxx"] = "01"
            if not ok:
