...
const WindAnimation = ({ map, windData, showWindAnimation, stormCells, selectedStation }) => {
  const windMarkersRef = useRef([]);

  useEffect(() => {
    if (!map || !showWindAnimation) {
    ...
    // Generate dynamic wind data based on storm cells and radar patterns
    const generateWindData = () => {
      const winds = [];
      
      // Add wind vectors around storm cells
      stormCells.forEach(storm => {
        const stormLat = storm.latitude;
        const stormLng = storm.longitude;
        
        // Generate wind vectors in a spiral pattern around the storm
        for (let radius = 0.2; radius <= 1.5; radius += 0.3) {
          for (let angle = 0; angle < 360; angle += 45) {
            const radian = (angle * Math.PI) / 180;
            const lat = stormLat + Math.cos(radian) * radius;
            const lng = stormLng + Math.sin(radian) * radius;
            
            // Calculate wind direction (cyclonic pattern)
            let windDirection = angle + 90; // Cyclonic rotation
            if (storm.tornadoProbability > 50) {
              windDirection += Math.sin(radius * 3) * 30; // Add turbulence for strong storms
            }
            
            const windSpeed = Math.min(120, 20 + storm.tornadoProbability * 1.2 + radius * 15);
            ...
            winds.push({
              ...
              speed: windSpeed,
              stormRelated: true,
              intensity: storm.tornadoProbability > 60 ? 'severe' : 'strong'
            });
          }
        }
      });
      
      // Add ambient wind patterns around selected station
      if (selectedStation && winds.length < 50) {
        const prevailingDirection = 240; // SW winds (common in US)
        const baseSpeed = 15;
        ...
        for (let i = 0; i < 30; i++) {
          ...
          winds.push({
            lat: selectedStation.latitude + Math.cos(angle) * distance,
            lng: selectedStation.longitude + Math.sin(angle) * distance,
            direction: prevailingDirection + (Math.random() - 0.5) * 60,
    ...
    // Generate and add new wind markers
    const winds = generateWindData();
    
    winds.forEach((wind, index) => {
      const arrowLength = Math.min(20, wind.speed / 3);
      const arrowColor = wind.stormRelated 
        ? (wind.intensity === 'severe' ? '#dc2626' : '#f97316')
        : '#3b82f6';
        ...
        html: `
          ...
          ">
            <div style="
              ...
              width: 0;
              height: 0;
              border-left: 6px solid ${arrowColor};
              border-top: 3px solid transparent;
              border-bottom: 3px solid transparent;
            "></div>
          </div>
        `,
        iconSize: [arrowLength, 3],
        iconAnchor: [arrowLength / 2, 1.5]
      });

      const windMarker = L.marker([wind.lat, wind.lng], {
        icon: arrowIcon,
        interactive: false,
        opacity: wind.stormRelated ? 0.9 : 0.6
      }).addTo(map);

      windMarker.bindTooltip(`
        <div style="font-size: 11px;">
          <strong>Wind:</strong> ${Math.round(wind.speed)} mph<br>
          <strong>Direction:</strong> ${Math.round(wind.direction)}°<br>
          <strong>Type:</strong> ${wind.stormRelated ? 'Storm-related' : 'Ambient'}
        </div>
      `, { sticky: true });

      windMarkersRef.current.push(windMarker);
    });

  }, [map, showWindAnimation, stormCells, selectedStation]);
...
const InteractiveRadarMap = ({ 
  ...
  const [playbackSpeed, setPlaybackSpeed] = useState(800); // ms per frame - slower for smooth crossfades
